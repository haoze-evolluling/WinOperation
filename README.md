...
# 🖥️ WinMaster — 本地系统管理工具

<img src="static/logo.png" width="120" alt="WinMaster Logo" />
...
**WinMaster** 是一款面向 Windows 系统的本地 Web 管理工具，采用 **Python Flask** 构建后端，通过浏览器提供图形化管理界面，实现对 Windows 系统的全方位监控与配置管理。
...
> ⚠️ **权限说明：** WinMaster 需要管理员权限运行，以执行系统级别的操作（如修改注册表、电源方案切换、系统激活等）。程序启动时会自动请求提权。
...
WinMaster 内置一个**独立于 Tab 切换的 30 秒后台刷新机制**，确保关键页面数据保持最新：
...
> WinMaster 支持 **深色/浅色** 双主题，通过个性化设置一键切换，全局生效。
...
```bash
# 1. 克隆仓库
git clone https://github.com/yourusername/WinMaster.git
cd WinMaster
...
输出位置：`dist/WinMaster.exe`

### 使用 PyInstaller 直接打包

```bash
pip install pyinstaller
pyinstaller WinMaster.spec
```
...
2. **管理员登录** — **点击页面标题「WinMaster」文字**弹出登录弹窗，输入内置账号或注册账号的密码
...
- Session Cookie 名称带随机后缀（`winmaster_session_` + 8 位随机 hex）
...
WinMaster 需要执行系统级操作，如修改注册表（Win11 优化、更新策略）、切换电源方案、激活系统等，这些操作需要管理员权限。程序启动时会自动请求提权。
...
删除 WinMaster 所在目录即可。如设置了定时关机计划，可在 WinMaster 中取消或通过 `schtasks.exe` 删除。
