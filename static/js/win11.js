...
async function loadSummary() {
  const data = await apiFetch("/api/win11/summary", { silent: true });
  if (!data || !data.success) return;
  const panel = document.getElementById("win11-summary-panel");
  if (!panel) return;
  const d = data.data;
  panel.innerHTML = `
    <div class="module-result-head">
      <strong>${d.enabled_count}/${d.total} 项已启用</strong>
      <span>${d.restart_recommended ? "建议重启资源管理器" : "无需额外操作"}</span>
    </div>
    <div class="module-state-list">
      ${(d.items || []).map(item => `
        <div class="${item.enabled ? "enabled" : ""}">
...
export function initWin11() {
  loadState();
  loadSummary();
  bindEvent("win11-summary-refresh", "click", loadSummary);
