...
def _service_status_python(name):
    try:
...
@update_bp.route("/api/update/health", methods=["GET"])
def update_health():
    expiry = _get_reg_value(UPDATE_KEY, "PauseUpdatesExpiryTime")
    start = _get_reg_value(UPDATE_KEY, "PauseUpdatesStartTime")
    no_auto_update = _get_reg_value(POLICY_KEY, "NoAutoUpdate")
    au_options = _get_reg_value(POLICY_KEY, "AUOptions")
    service_rows = [_service_status_python(name) for name, _ in SERVICES]

    paused = bool(expiry)
    disabled_services = [
        row for row in service_rows
        if str(row.get("start_type", "")).lower() in ("disabled", "manual")
        or str(row.get("status", "")).lower() == "stopped"
    ]

    recommendations = []
    if paused:
        recommendations.append("更新暂停策略已写入，适合需要稳定环境的维护窗口。")
    else:
        recommendations.append("当前未检测到长期暂停策略，系统可能按默认策略接收更新。")
    if no_auto_update == "1":
        recommendations.append("组策略 NoAutoUpdate 已启用。")
    if len(disabled_services) < len(SERVICES):
        recommendations.append("部分更新相关服务仍可能运行，必要时可重新执行推迟操作。")
    else:
        recommendations.append("核心更新服务均处于停止或非自动启动状态。")
