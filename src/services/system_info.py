...
def get_gpu_names():
    """Return list of dicts: [{'name': '...'}, ...]."""
    try:
        wmi_svc = _wmi()
        gpus = []
        for gpu in wmi_svc.ExecQuery("SELECT Name FROM Win32_VideoController"):
...
def get_memory():
    ...
    try:
        ...
        available = round(mem.available / (1024 ** 2), 1)
        used = round((mem.total - mem.available) / (1024 ** 2), 1)
