...
@system_bp.route("/api/system/process-summary", methods=["GET"])
def system_process_summary():
    process_count = 0
    thread_count = 0
    top_memory = []

    try:
        for proc in psutil.process_iter(["pid", "name", "num_threads", "memory_info", "cpu_percent"]):
            try:
                info = proc.info
                process_count += 1
    ...
    top_cpu = sorted(top_memory, key=lambda item: item["cpu"], reverse=True)[:8]
    top_memory.sort(key=lambda item: item["memory_mb"], reverse=True)
