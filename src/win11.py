...
def get_classic_menu():
    return jsonify({"success": True, "data": {"enabled": _classic_menu_enabled()}})
...
@win11_bp.route("/api/win11/summary", methods=["GET"])
def win11_summary():
    classic_menu = _classic_menu_enabled()
    taskbar_left = _get_reg_dword(TASKBAR_PATH, "TaskbarAl") == 0
    search_disabled = _get_reg_dword(SEARCH_PATH, "DisableSearchBoxSuggestions") == 1
    items = [
        {
            "id": "classic_menu",
            "label": "经典右键菜单",
            "enabled": classic_menu,
            "detail": "减少进入二级菜单的点击成本。" if classic_menu else "当前保持 Win11 默认右键菜单。",
        },
        {
            "id": "taskbar_left",
            "label": "任务栏左对齐",
            "enabled": taskbar_left,
            "detail": "更接近 Windows 10 操作肌肉记忆。" if taskbar_left else "当前使用居中任务栏。",
        },
        {
            "id": "search_disabled",
            "label": "禁用联网搜索建议",
            "enabled": search_disabled,
            "detail": "开始菜单优先搜索本地内容。" if search_disabled else "开始菜单可能展示联网搜索建议。",
        },
    ]
    ...
    return jsonify({
        ...
        "data": {
            ...
            "total": len(items),
            "restart_recommended": enabled_count > 0,
        },
    })
